...
export default function FundingHistory({ personalId }) {
  ...
  React.useEffect(() => {
    const fetchFundings = async () => {
      try {
        const response = await axios.get(`${config.apiUrl}/fundings?userId=${personalId}`);
          ...
          {(rowsPerPage > 0
            ...
            <TableRow key={row.fundingId}>
              <TableCell component="th" scope="row" sx={{ fontFamily: 'Montserrat' }}>
                <Link href={`/projects/${row.projectId}`} className={styles.link}>
                  {row.projectName}
                </Link>
              </TableCell>

              <TableCell style={{ width: 160 }} align="center" sx={{ fontFamily: 'Montserrat' }}>
                {row.projectCategory}
              </TableCell>
              <TableCell style={{ width: 160 }} align="center" sx={{ fontFamily: 'Montserrat' }}>
                {row.amount}
